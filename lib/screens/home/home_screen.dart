...
class _HomeScreenState extends State<HomeScreen> with SingleTickerProviderStateMixin {
  ...
  bool _hasTriggeredLunaCall = false;
    ...
    showDialog(
      ...
      builder: (context) {
        ...
        return AlertDialog(
          ...
          title: const Text("Luna is calling you"),
          content: const Text("We noticed high stress. Want to talk?"),
          ...
          actions: [
            ...
            ElevatedButton(
              ...
              onPressed: () {
                ...
                Navigator.push(
                  context,
                  MaterialPageRoute(builder: (_) => const LunaCallScreen()),
                );
              },
                  ...
                  children: [
                    ...
                    StreamBuilder<List<StressEntry>>(
                      ...
                      builder: (context, snapshot) {
                        ...
                        if (snapshot.hasData && snapshot.data!.isNotEmpty) {
                          ...
                          if (stressVal >= 80) {
                            if (!_hasTriggeredLunaCall) {
                              _hasTriggeredLunaCall = true;
                    ...
                    _ActionButton(
                      ...
                      onTap: () {
                        Navigator.push(
                          context,
                          MaterialPageRoute(builder: (_) => const FinalAnalysisScreen()),
                        );
                      },
                    ),
                    ...
                    LayoutBuilder(
                      builder: (context, constraints) {
                        return GridView.count(
                          ...
                          mainAxisSpacing: 12,
                          crossAxisSpacing: 12,
                          childAspectRatio: constraints.maxWidth > 400 ? 1.3 : 1.15,
                          children: [
                            _FeatureCard(
                              title: 'Voice Check',
                              icon: Icons.mic,
                              emoji: '🎙️',
                              gradient: const [Color(0xFFE8F5E9), Color(0xFFD0F0E8)],
                              darkGradient: const [Color(0xFF1B3A2D), Color(0xFF1A2E28)],
                              onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const VoiceAnalysisScreen())),
                            ),
                            _FeatureCard(
                              title: 'Text Check',
                              icon: Icons.edit,
                              ...
                              darkGradient: const [Color(0xFF1A2A3A), Color(0xFF1A2535)],
                              onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const TextAnalysisScreen())),
                            ),
                            _FeatureCard(
                              title: 'Face Check',
                              icon: Icons.camera_alt,
                              ...
                              darkGradient: const [Color(0xFF3A1A2A), Color(0xFF351A28)],
                              onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const FaceAnalysisScreen())),
                            ),
                            _FeatureCard(
                              title: 'AI Chat',
                              icon: Icons.smart_toy,
                              ...
                              darkGradient: const [Color(0xFF2A1A3A), Color(0xFF251A35)],
                            ...
                            _FeatureCard(
                              ...
                              darkGradient: const [Color(0xFF3A2A1A), Color(0xFF352A1A)],
                            ...
                            _FeatureCard(
                              ...
                              darkGradient: const [Color(0xFF1A2E3A), Color(0xFF1A2835)],
...
class _LunaAssistantCard extends StatelessWidget {
  const _LunaAssistantCard();
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: LinearGradient(
          colors: isDark 
              ? [const Color(0xFF1B3A2D), const Color(0xFF1A2E28)]
              : [const Color(0xFFE8F5E9), const Color(0xFFD0F0E8)],
          begin: Alignment.topLeft,
          end: Alignment.bottomRight,
        ),
        borderRadius: BorderRadius.circular(20),
        boxShadow: [
          BoxShadow(
            color: const Color(0xFF9BE7C4).withOpacity(isDark ? 0.05 : 0.3),
            blurRadius: 10,
            offset: const Offset(0, 4),
          ),
        ],
          ...
          Row(
            children: [
              Container(
                ...
                width: 52,
                decoration: BoxDecoration(
                  color: isDark ? const Color(0xFF2A4A3D) : Colors.white.withOpacity(0.6),
                  shape: BoxShape.circle,
                ),
                child: const Icon(Icons.smart_toy, color: Color(0xFF43A047), size: 28),
              ),
            ...
            children: [
              Expanded(
                child: ElevatedButton.icon(
                  style: ElevatedButton.styleFrom(
                    backgroundColor: isDark ? const Color(0xFF2A4A3D) : Colors.white,
              ...
              Expanded(
                child: ElevatedButton.icon(
                  ...
                  onPressed: () {
                    Navigator.push(context, MaterialPageRoute(builder: (_) => const LunaCallScreen()));
                  },
...
class _MiniStressChart extends StatelessWidget {
  ...
  const _MiniStressChart({required this.cardColor, required this.textColor, required this.subtextColor});
  ...
  static List<Color> _barGradient(double stress) {
    if (stress >= 75) return const [Color(0xFFFF5252), Color(0xFFD32F2F)];
    if (stress >= 50) return const [Color(0xFFFFB74D), Color(0xFFF57C00)];
    if (stress >= 25) return const [Color(0xFF81C784), Color(0xFF43A047)];
    return const [Color(0xFF9BE7C4), Color(0xFF7AD7C1)];
  }
    ...
    return Container(
      ...
      child: Column(
        children: [
          Row(
            children: [
              Icon(Icons.show_chart_rounded, color: isDark ? const Color(0xFF9BE7C4) : const Color(0xFF7AD7C1), size: 22),
              const SizedBox(width: 8),
              Text('Stress Trend', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600, color: textColor)),
              const Spacer(),
              Text('Last 3', style: TextStyle(fontSize: 12, color: subtextColor)),
            ],
          ),
          const SizedBox(height: 16),
          StreamBuilder<List<StressEntry>>(
                ...
                return Container(
                  ...
                  child: Center(
                    child: Column(
                      mainAxisSize: MainAxisSize.min,
                      children: [
                        Icon(Icons.insert_chart_outlined_rounded, size: 32, color: subtextColor.withValues(alpha: 0.5)),
                        const SizedBox(height: 6),
                        Text(
                          'No stress data yet.\nRun an analysis to see your trend.',
                          textAlign: TextAlign.center,
              ...
              return SizedBox(
                ...
                child: CustomPaint(
                  painter: _TrendLinePainter(
                    entries: displayEntries,
                    barAreaHeight: barAreaHeight,
                    lineColor: isDark ? Colors.white.withValues(alpha: 0.15) : Colors.grey.withValues(alpha: 0.2),
                  ),
                  child: Row(
                    crossAxisAlignment: CrossAxisAlignment.end,
                    children: List.generate(displayEntries.length, (i) {
                      final e = displayEntries[i];
                      final barHeight = (e.combinedStress / 100.0 * barAreaHeight).clamp(12.0, barAreaHeight);
                        ...
                        child: Padding(
                          ...
                          child: Column(
                            ...
                            children: [
                              Text(
                                '${e.combinedStress.toStringAsFixed(0)}%',
                                style: TextStyle(fontSize: 13, fontWeight: FontWeight.w700, color: _barColor(e.combinedStress)),
                              ),
                              const SizedBox(height: 4),
                              TweenAnimationBuilder<double>(
                                tween: Tween<double>(begin: 0, end: barHeight),
...
class _TrendLinePainter extends CustomPainter {
  ...
  _TrendLinePainter({required this.entries, required this.barAreaHeight, required this.lineColor});
  ...
  @override
  bool shouldRepaint(covariant _TrendLinePainter oldDelegate) => entries != oldDelegate.entries;
}
...
class _EmergencyBannerState extends State<_EmergencyBanner> {
  ...
  Future<void> _autoTriggerAlert() async {
    ...
    try {
      final sent = await EmergencyService().triggerEmergencyAlert(widget.stressLevel);
      if (mounted) setState(() => _alertSent = sent);
    } catch (_) {}
  }
        ...
        children: [
          ...
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                const Text('⚠️ High Stress Alert', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w700, fontSize: 15)),
                const SizedBox(height: 2),
                Text(
                  _alertSent
                      ? 'Stress at ${widget.stressLevel.toStringAsFixed(0)}%. Emergency contacts have been notified.'
...
class _FeatureCard extends StatefulWidget {
  ...
  const _FeatureCard({required this.title, required this.icon, required this.emoji, required this.gradient, required this.darkGradient, required this.onTap});
...
class _FeatureCardState extends State<_FeatureCard> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    super.initState();
    _scaleCtrl = AnimationController(vsync: this, duration: const Duration(milliseconds: 120), lowerBound: 0.0, upperBound: 0.04);
    _scaleAnim = Tween<double>(begin: 1.0, end: 0.96).animate(_scaleCtrl);
  }
      ...
      child: AnimatedBuilder(
        ...
        child: Container(
          decoration: BoxDecoration(
            ...
            boxShadow: [
              BoxShadow(color: widget.gradient.first.withOpacity(isDark ? 0.1 : 0.2), blurRadius: 8, offset: const Offset(0, 3)),
            ],
          ),
          child: Column(
            mainAxisAlignment: MainAxisAlignment.center,
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Row(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            Icon(icon, color: isDark ? const Color(0xFF9BE7C4) : Colors.redAccent),
            const SizedBox(width: 8),
